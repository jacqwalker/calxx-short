...
$roletype_error = $sectorexperience_error = "";
...
if(isset($_POST["submit"])) {
    ...
    if (empty($_POST["email2"])) {
      $email2_error = "Website address is required";
    } else {
      $email2 = test_input($_POST["email2"]);
      // check if e-mail address is well-formed
      if (!preg_match("/\b(?:(?:https?|ftp):\/\/|www\.)[-a-z0-9+&@#\/%?=~_|!:,.;]*[-a-z0-9+&@#\/%=~_|]/i",$email2)) {
        $email2_error = "Invalid website link";
      }
    }
